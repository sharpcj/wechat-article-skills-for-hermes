...
description: 自动搜索、抓取、改写技术内容，生成适合微信公众号的中文科普文章，并生成主题封面图。涵盖 AI 大模型、GitHub 开源工具、技术话题。当用户说"写一篇关于 XXX 的（公众号/技术）文章"、"帮我研究 XXX 并写成文章"、"生成公众号文章"、"科普一下 XXX"时，应使用本技能；即使用户没有明说"公众号"，只要是要把某个技术话题写成一篇面向大众的中文文章，也应触发。
---
...
1. **🎯 封面图是强制要求**
   - 每篇文章必须生成一张吸引人的主题封面图
   - 封面图**不嵌入 Markdown 正文**，文件命名 `cover.png` 与文章 .md 同目录，发布时由 `publisher.py --cover cover.png` 单独上传

2. **📝 标题简洁有吸引力**
   - 公众号标题不能太长太啰嗦，需要具备吸引力，让读者有兴趣点开
   - 避免平铺直叙的陈述句，用悬念、数字、对比、提问等方式增加点击欲
   - 示例：❌「Claude Agent SDK 的功能介绍与使用指南」→ ✅「Claude Agent SDK：AI 真正的生产力开关」
   - 标题在 Markdown 中用 `#` 写出，格式化时会被移除（公众号后台单独填写），但写作时仍需认真拟定

3. **图片文字语言**
   - 图片中的文字根据内容和场景选择合适的语言，不强制使用中文
   - 面向中文读者的公众号文章，优先使用中文；涉及代码、术语、品牌名等可保留原文
...
使用 `web_extract` 工具（或 `browser_navigate`）获取2-5篇代表性文章的完整内容：
...
封面图**首选用本地 ComfyUI 生成**（更可控、用本地算力）。调用本技能自带的 `scripts/comfyui_gen.py`：
...
阅读文章内容
  ...
  ├─ 有教程/步骤？     → process（流程步骤）⚠️ 不用 ComfyUI，用 Mermaid 或跳过
  ├─ 有知识框架？      → mindmap（思维导图）
  ├─ 有发展历程？      → timeline（时间线）
...
使用 `write_file` 工具创建Markdown格式的文章文件：
...
- ❌ 不要在 Markdown 里写封面图（如 `![](cover.png)`），封面图发布时单独上传
- ✅ 只输出：标题 + 正文 + 总结
...
### 格式规范
- [ ] 封面图 `cover.png` 已生成并保存到文章同目录，但**未写入 Markdown**（发布时由 `--cover` 上传）
...
### 实用脚本（可选/高级）
- **scripts/generate_image.py** - 备用生图脚本（支持 Gemini/DALL-E，需自备 API key）。一般情况下优先用 Hermes 内置的 `image_generate` 工具，无需此脚本。
- **scripts/generate_cover_optimized.py** - 优化的封面图生成脚本（备用）
- **scripts/generate_temp.py** - 临时图片生成工具（备用）
...
生图**首选本地 ComfyUI**（调用本技能自带的 `scripts/comfyui_gen.py`）：

**生成封面图（2.35:1）**：
```bash
python3 scripts/comfyui_gen.py --prompt "提示词" --workflow templates/image_z_image.json --width 1024 --height 432
```

**生成方形/竖版配图**：
```bash
python3 scripts/comfyui_gen.py --prompt "提示词" --workflow templates/image_z_image.json --width 1024 --height 768   # 内容图 4:3
```
...
# 调用本技能自带的脚本：
#   python3 scripts/comfyui_gen.py --prompt "..." --workflow templates/image_z_image.json --width 1024 --height 432
# 备选：image_generate(prompt="...", aspect_ratio="landscape")
生成封面图(prompt="Claude Sonnet 4 封面，蓝紫渐变，标题+副标题...")

# 步骤6：内容配图（按需）
if 文章有性能对比数据:
    # 同样首选 comfyui_gen.py（封面 --width 1024 --height 432），备选 image_generate
    生成配图(prompt="性能对比柱状图，Claude vs GPT vs Gemini...")
...
3. **控制图片数量** - 按需配图，每个 H2 章节最多 1 张，宁缺毋滥
