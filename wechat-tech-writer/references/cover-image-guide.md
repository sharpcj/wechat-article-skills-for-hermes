...
**首选用本地 ComfyUI 生图**（调用本技能自带的 `scripts/comfyui_gen.py`；调用前确认 ComfyUI 在线，`curl` 检测返回 200）：
...
脚本把生成图片的绝对路径打到 stdout，取最后一行用于文章封面即可。封面图比例 **2.35:1**（宽幅），尺寸 **1024×432**（ComfyUI 上限 1024×1024，已取 8 的倍数）。

> 备选：ComfyUI 不可用时改用 `image_generate(prompt=..., aspect_ratio="landscape")`，工具返回图片的本地路径或 URL，直接用于文章封面即可。`aspect_ratio` 取值：`landscape`（16:9，封面常用）、`square`（1:1）、`portrait`（16:9 竖版）。

**生成质量建议**：
- 提示词里务必反复强调简体中文要求（`text in simplified Chinese, accurate`），生图模型对中文字形支持有限，这能降低乱码概率。
- 若中文反复出错，退一步把文字留到排版阶段叠加，或减少封面上的中文字数。

**备选方案（高级）**：仓库内 `scripts/generate_image.py` 仍保留了 Gemini/DALL-E 的调用能力，需自备 `GEMINI_API_KEY` 或 `OPENAI_API_KEY`。常规场景无需使用，详见 [api-configuration.md](api-configuration.md)。

**错误处理**：
- 生成失败 → 简化提示词，减少复杂要求
- 中文乱码 → 强化提示词中的中文要求，或减少封面文字
...
| 问题 | 调整方案 |
|------|---------|
| 文字模糊/乱码 | 强化提示词：large, clear Chinese characters；或减少封面文字 |
...
2. 如果仍然失败，减少封面上的中文字数，或把文字留到排版阶段叠加
...
**原因**：提示词未强调高质量，或主题过于复杂

**解决**：
- 在提示词中加入 `high quality, professional design, sharp details`
- 简化构图，让模型把细节集中在主体上
- 生成后检查文件大小，应 > 200KB
