...
## ⚠️ 不适合 ComfyUI 的 Type

以下 Type 用 ComfyUI 生成效果差（文字排版混乱、线条不清晰），**不要用 ComfyUI 生成**：

- **process（流程步骤）**：改用 Mermaid 等纯软件绘图工具，或直接跳过不配图
- **whiteboard（板书白板）**：同上

## 图片内文字语言

公众号配图画面中的文字根据内容和场景选择合适的语言：

- 面向中文读者的文章，优先使用中文
- 涉及代码、术语、品牌名等可保留原文
- 在 prompt 中直接写出要显示的文字内容，不要只写 "labels in Chinese" 等元描述
- Prompt 本身可中英混排
...
如画面中需要文字：大而醒目，手写风字体优先，仅保留核心关键词。根据内容和场景选择语言，在 prompt 中写出要显示的文字原文。
